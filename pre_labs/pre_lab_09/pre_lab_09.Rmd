...
This is my link. https://datawrapper.dwcdn.net/nGN0C/1/
...
```{r}
#Total amount of voters per county
md_gov_county <- md_gov_county |> 
  mutate(Total_County = Cox + Moore + Lashar + Wallace + Harding + `Write-ins`) 
```

```{r}
#Moore percent
md_gov_county <- md_gov_county |>
  mutate(Moore_Percent = Moore/Total_County * 100)
```
...
Before, we saw the entire U.S. map filled with Grey areas, except Maryland. Now,
we only see Maryland by counties without the surrounding states. We also see which
county has the largest percentage of Moore voters.
...
Before, we would only see the FIP Code and a very long version of the percentages
of Wes Moore. Now we can see the names of each county and a more short version of the
percentages of the counties.
...
https://datawrapper.dwcdn.net/jrSIP/1/
