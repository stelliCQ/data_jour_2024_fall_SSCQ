...
```{r}
voters_by_county <- read_csv("images/tabula-Eligible Active Voters by County-PP24.csv")
...
The problem with this table is that the Poll Book Totals answers are viewed by R to be the columns of this dataframe. Furthermore, we have a lot of NAs as well. 

```{r}
benzie_2024 <- read_csv("images/tabula-Benzie Aug 6,2024 Official Precinct Results.csv")
...
The issue is that the columns are still in the rows and the columns don't have a value attached yet. 

```{r}
benzie_2024 <- read_csv("images/tabula-Benzie Aug 6,2024 Official Precinct Results.csv", col_names = FALSE)
...
Now, the columns are allocated to their specific candidates and can see the precint results of the county. The issue that remains is that in the next page there are still columns with the X and number as their name, which causes problems when it comes to adding the numbers up for analysis. Since what is column X12 measuring? We would need to clean this further.
