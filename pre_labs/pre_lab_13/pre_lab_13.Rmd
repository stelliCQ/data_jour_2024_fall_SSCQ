...
```{r}
create_credentials(GROQ_API_KEY = "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")
```
...
The first 10 names have more cultural symbolism and pop culture references.
The last 10 names are more focused on personality and physical features.
...
Llama got the people's names and years, but not the rest of the highlighted 
boxes.
...
Llama didn't get it all right. I think it needed to specific what portfolio meant
and what hill_experience also meant. I think for portfolio it needs to have a
code that explicitly tells you what you need for the portfolio. 
...
The first response didn't make any sense to me since it was using terminology
and other code that I don't know about. The second time the explanation made more
sense.However, how the answer changes depending on when you re-run it is worrying.
...
There are some parts of the code that makes sense and others that doesn't.
For example, these lines of code are useless since the filter is redundant, and 
the summarize calculation doesn't give you a good measure of age since mean
is influenced heavily by outliers. These make assumptions that are seem incorrect
and confused me more than helped me ngl.

### filter(registration_date > dob) |>
  summarise(
    num_voters = n(),
    mean_age = mean(difftime(registration_date, dob, units = "days") / 365.25)
  )###
...
My code for this isn't working... I got the image there but not the description of this. 


```{r}
knitr::include_graphics(rep("images/fundraising_email.png"))
...
```{r}
# Path to the image
image_path <- "images/fundraising_email.png"  # Change this to your actual image path

# Display the image in the report
knitr::include_graphics(image_path)

# Make the API call to describe the image
try({
  military_response <- axolotr::ask(
    prompt = "Describe the things in this image, being as specific as possible.",
    model = "llama-3.2-11b-vision-preview",
    image = image_path  # Provide the correct path to the image
  )
  
  # Print the model's response
  cat(military_response)
}, silent = FALSE)  # Set silent = TRUE to suppress error messages
```
...
In a way the code did run and work, but it didn't make a lot of sense to me because 
it confused things that could be giveaways but weren't. So it wasn't super 
successful for me at least.
