...
The warning message says it has one or more parsing issues. e.g. dat <- vroom (...) problems(dat.)
...
There seems to be 1,640 rows of problems. It seems that there's some issues that the data inside these specific rows aren't able to be understood by the machine. Col means columns and columns 12 and 13, are about provisional and limited voting, so maybe its suppose to read as an intergers instead of true or false.

```{r}

problems(texas_precinct_20)
colnames(texas_precinct_20)
...
It doesn't show any parsing errors.
...
The district field is the data type <dbl>, "precinct" is in <chr>, and mail is in <dbl>. 
...
Question for Little in class: What does 5+ do? What would be the difference if i use 6+?
...
There is 402,345 mail votes that are NA. This could mean that the counties didn't report it at all as another category or something else happened.
...
For January 1900 there are 12 registrations and one for May 1900. The other 18 results look plausible that people who registered at that time could be alive currently. Maybe the Jan. 1900 are dead and haven't been taken out of the system?
...
I think legacy data would be interesting to explore since I have no idea what that means and what could it entail in the elections, and there's 2,202 people in it, which could be impactful. 
...
There are 14 rows in this data.
...
Amount is in <chr> (character), It should be in <dbl> though. 
...
A. Some Zip codes are 9 digits and others 5. 
B. Six of these rows come from the same address and person. 
C. The name of the city is spelled wrong in some rows.
D. The column names are spelled different from others. Like LAST_NAME is in all caps and first name isn't.
...
The code standarizes it in a more uniform way. For example, last_name is in low caps unlike before. Column "1_linenumber has an x before the 1_. 
...
The error says that inside the summarise function we cannot add the total amount due to the fact that amount is a character. 
...
There are 2 duplicates, and the donor's name is Derrick Hamilton.
...
Now the Zip is five digits uniformly and doesn't have a dash (-). 
...
There are two mispellings left.
...
Now there's no misspellings left of Conowingo. 
