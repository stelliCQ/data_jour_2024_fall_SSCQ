...
The columns that exist in this data are the coordinates, length and area of the precincts in "shape". The columns also include information like school, congress, and legislative districts of each precincts.
...
It contains information regarding the unique precinct ID, name and polling ID, and what Congressional district, school board seat, and legislative seats. 
...
The output is a map of the precints of PG County. 
...
The output is a filtered version of the counties of Maryland. 
...
------------
```{r}
pr_municipality <- counties |> 
  filter(STATEFP == "72")
```

```{r}
##This map came out weird with the coordinates. Ask Ryan about it
pr_municipality |>
  ggplot() +
  geom_sf() +
  theme_minimal()
...
Now, we have a map of Maryland by county.
...
We now have a better definition of the county lines, and there's no longer ones 
left in blank.
...
The output gives us the total of turnouts of the may primaries for early voting, absentee,
provisional, and eligible voters.
...
We have now jointed the precint turnount data with the PG precint geographical data.
...
Now we see the amount of total eligible people in each precinct in PG County by 
shades of blue.
...
We see that Montgomery, PG, Baltimore County and Anne Arundel have the most voters
...
We see the estimated population for each county in alphabetical order.
...
We now have both the geometry with the coordinates of the GEOID of the population 
estimates by county.
...
We are now viewing both dataframes we want to join. We need to clean the county
name columns so we can join them.
...
We now have only the county names and have the population column be named
"population", and not estimate. 
...
Now, both dataframes are joined together, but two counties are coming out with NA 
from this join: Baltimore and St. Mary's.
...
Now, we told R that St. Mary's and Saint Mary's (for example) are the same thing,
...
Now we have a completed table that shows the total population, the total voters
elibible and the total voters per 10,000 residents in each county. We see that the
top county with total voters per 10k are Worcester, Queen Anne's and Talbot, which
aren't the top most populated counties in MD. 
...
We now have a county map of Maryland filled by the amount of voters pero 10,000
population.
...
With this we see a clearer distinction between counties different results compared 
to the previous map. The counties with the most and least stand out better. 
...
We see that the most of the central region of Maryland has the same rate of voters per 10k.
...
We now have a new columns that measures the amount of unaffiliated voters per
10K population. 
...
We see that for the central region of Maryland has the largest amount of unaffiliated
voters per 10K. We see that PG, Baltimore City and Somerset has the smallest amount
of UNA voters per 10k.
