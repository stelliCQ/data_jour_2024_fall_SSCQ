...
###There's a folder in the prelab so that's why there's data/. 
...
###Colnames --> Retrieve or set the row or column names of a matrix-like object. Tells us the column tabs
...
glimpse(primary_24)
```
##Mutate is telling R HEY do math to all these columns that are dbl. It creates a new column name in percent_election_day
...
###The code above is creating a new column using mutate, which combines the numerical values of the columns votes and election day to create a new output. We are also naming this new column that was created via mutate. 
...
###SELECT IS FILTER THE COLUMNS We want. 
...
The results changed since first this is being arranged by the percentage of election day by multiplying by 100. It also changed since now the order is from the person who had the highest percentage of votes on election day, instead of alphabetical order in the previous code.  
...
**Answer**
###It's not really doing anything different from the code above. Since we could've found this answer by going to the past page(Previous if that's what the 1,2...90 Next is called). 
...
###THIS GAVE ME 4 variables. (Note that mutate doesn’t literally combine similar records together - you’ll still need to do another group_by and summarize block - but it does make your data more accurate.)
...
A. str_to_lower() --> converts to lower case.
B. str_to_title(). It turns it that only the first letter of the sentence is capitalized. 
...
###The largest contribution came from amount category of less than $100.
...
It's important since it makes it easier to analyze a section of the data more closely instead of the entire dataset. If we want to write a story about PG County, this makes it easier.
...
That gives us 17 candidates. But the code is repetitive, no? We can do better using boolean operators -- AND and OR. In this case, AND is `&` and OR is `|`. you can use & instead of pike.
...
The expenditure column now says <date>, meaning it no longer views it as a character. The dates are now separated in a different format.  
...
The data is showing the first day of each month and how much money was accumulated in that time frame. This is also showing us the months that had the most expenditures earned. 
