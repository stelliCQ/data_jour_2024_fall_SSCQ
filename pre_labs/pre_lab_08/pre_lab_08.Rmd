...
**Answer**
The first lines of code mutates a new column by subtracting the unaffiliated voters
of 2024 with the 2022 ones. Then we create a new dataframe that arranges the 
new column, and selects the two columns we want to see: County and UNAF_DIFF. 
Lastly, we limit the results shown below to 10. 
...
The only thing that appears is a gray box that will hold the graph.
...
We see a bar chart of the 10 counties with the largest difference in UNAF Voters, with their names at the bottom and the values on the left side. These are not in order of largest to greatest,but
by the alphabet. 
...
Now, the counties are in order of least to greatest. 
...
Now, the counties are on the Y-axis and the values on the X-axis.Makes it easier 
to read. 
...
Now the grey box beneath the columns is gone. 
...
The chart has a different font, background color, and the letters are bigger.
...
The graph now has a title, a caption, and a well-defined values for the
x and y axis. x
...
We see the total contributions of every day from April to June for Republicans running for offices around 
the country by grouping and summarizing so it only shows these 2 values. 
...
Now we have a line graph that visualizes the ups and downs of contributions during the 4 months of 
recorded data. We see a big increase at the end of July. We see that this is measuresd by the
contributions on the y-axis and the dates on the x-axis.
...
This graph is now showing the values on the x-axis by week instead of by month, this is done
by making sure that the labels for the date are shown by day and month.
...
Now the dates at the x-axis are titled so that it can be more easy to read.
