...
Yes, all the csv files have the same column structure. 
...
### Task 3: Using bind_rows *Added to reference Notebook*
...
Largest county population is Montgomery County.
...
There are 24 rows and we expected 24 rows.
...
The message below said "joining with 'by=join_by(COUNTY)'".
...
The largest county with the population of black people over 18 is PG County with them being 78% Democrats.
...
The county with the largest percentage of Asian population that's 18+ is Howard, which 52% are Democrats. There's a lower percentage of Asians over the age of 18 registered to vote compared to African Americans by county.
The same thing occurs that there is a lower percentage Asians that are 18+ and Democrats if we compare to the previous code. 
...
The ZCTA5N is the <dbl> datatype, which means that its identified like its integers. 
...
The datatype is now a character.
