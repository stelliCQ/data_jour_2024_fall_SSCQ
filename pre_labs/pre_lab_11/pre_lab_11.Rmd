...
All of the columns are saved as character <chr>, meaning we cant do math with 
the results until they're converted. It also has commas which makes it a little
harder to make the numbers do math. 
...
It's referring that the column that shows up in the HTML in order (2-6) is named
by candidate.
...
There are 7 <ul> tag in the HTML source. I don't think that the output suggests
how many ul there are since 10 <li> lines can be in either one <ul> or in 10.
I think inspecting <ul> specifically is better.
...
We see in the output the committees in the output and other resources like the
...
Now we have a clean list of all the names of the committeees in Congress. 
...
#NOW we have the link to every specific committee.
